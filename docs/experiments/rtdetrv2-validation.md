...
Verify the checkpoint SHA against `config/model-profiles.json` before export.
...
external Python environment from the locked `tools/rtdetrv2-export/uv.lock`,
...
Run this outside the AMD SIF, with a pre-built interpreter containing the
locked export dependencies:
...
"${OVG_RTDETR_EXPORT_PYTHON}" tools/phase2-assets build-rtdetrv2-r50 \
