...
using isaac_ros_tensor_list_interfaces::msg::Tensor;
using isaac_ros_tensor_list_interfaces::msg::TensorList;
using nvidia::isaac_ros::yolov8_std::DecodeYoloV8TensorList;
using nvidia::isaac_ros::yolov8_std::kGxfFloat32;
using nvidia::isaac_ros::yolov8_std::YoloV8DecoderConfig;
  ...
  int data_type = kGxfFloat32)
{
  ...
  tensor.shape.rank = static_cast<int32_t>(shape.size());
...
TensorList MakeTensorList(const Tensor & tensor)
{
  TensorList msg;
...
TEST(YoloV8DecoderTest, DecodesSingleDetection)
{
  ...
  const auto msg = MakeTensorList(MakeTensor("output_tensor", {1, 6, 2}, values));

  const auto detections = DecodeYoloV8TensorList(msg, Config());
...
TEST(YoloV8DecoderTest, AppliesConfidenceThreshold)
{
  const auto values = MakeYoloOutput(
    ...
    {0.10F}, {0.20F});
  const auto msg = MakeTensorList(MakeTensor("output_tensor", {1, 6, 1}, values));
...
TEST(YoloV8DecoderTest, AppliesNms)
{
  const auto values = MakeYoloOutput(
    ...
    {0.90F, 0.80F, 0.70F},
    {0.10F, 0.10F, 0.10F});
  const auto msg = MakeTensorList(MakeTensor("output_tensor", {1, 6, 3}, values));
...
TEST(YoloV8DecoderTest, MissingTensorThrows)
{
  const auto values = MakeYoloOutput(
    {100.0F}, {120.0F}, {40.0F}, {50.0F},
    {0.10F}, {0.90F});
  const auto msg = MakeTensorList(MakeTensor("other_tensor", {1, 6, 1}, values));

  EXPECT_THROW(DecodeYoloV8TensorList(msg, Config()), std::runtime_error);
}
...
TEST(YoloV8DecoderTest, BadDtypeThrows)
{
  const auto values = MakeYoloOutput(
    {100.0F}, {120.0F}, {40.0F}, {50.0F},
    {0.10F}, {0.90F});
  const auto msg = MakeTensorList(MakeTensor("output_tensor", {1, 6, 1}, values, 7));
...
TEST(YoloV8DecoderTest, BadShapeThrows)
{
  const auto values = MakeYoloOutput(
    {100.0F}, {120.0F}, {40.0F}, {50.0F},
    {0.10F}, {0.90F});
  const auto msg = MakeTensorList(MakeTensor("output_tensor", {6, 1}, values));
...
TEST(YoloV8DecoderTest, BadByteSizeThrows)
{
  ...
  const auto msg = MakeTensorList(tensor);
