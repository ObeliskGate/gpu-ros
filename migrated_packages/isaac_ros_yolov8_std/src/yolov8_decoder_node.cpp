...
#include "isaac_ros_yolov8_std/yolov8_decoder_node.hpp"
...
void YoloV8DecoderNode::InputCallback(const TensorList::SharedPtr msg)
{
  try {
    pub_->publish(DecodeYoloV8TensorList(*msg, config_));
  } catch (const std::exception & error) {
    RCLCPP_ERROR(get_logger(), "Failed to decode YOLOv8 TensorList: %s", error.what());
  }
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(nvidia::isaac_ros::yolov8_std::YoloV8DecoderNode)
