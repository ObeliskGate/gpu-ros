...
TEST(ImagePreprocessPlan, ComputesLetterboxGeometry)
{
  ...
  const auto plan = nvidia::isaac_ros::detection_common::MakeImagePreprocessPlan(
    image, 8, 8,
    nvidia::isaac_ros::detection_common::PreprocessNormalization::kNone);
  EXPECT_EQ(plan.resized_width, 8);
  EXPECT_EQ(plan.resized_height, 4);
  EXPECT_EQ(nvidia::isaac_ros::detection_common::SourceRowBytes(plan), 12U);
}
...
TEST(ImagePreprocessPlan, RejectsShortStepAndBuffer)
{
  ...
  EXPECT_THROW(
    nvidia::isaac_ros::detection_common::MakeImagePreprocessPlan(
      image, 8, 8,
      nvidia::isaac_ros::detection_common::PreprocessNormalization::kNone),
...
TEST(ImagePreprocessPlan, CpuReferenceUsesRgbNchwAndExactZeroPadding)
{
  ...
  const auto plan = nvidia::isaac_ros::detection_common::MakeImagePreprocessPlan(
    image, 2, 2,
    nvidia::isaac_ros::detection_common::PreprocessNormalization::kUnitRange);
  const auto values = nvidia::isaac_ros::detection_common::ExecuteCpuPreprocess(image, plan);
