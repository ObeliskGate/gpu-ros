...
| `gpu_ros_tensor_bundle_msgs` | Project-owned TensorBundle message definitions | Yes |
| `gpu_ros_nvidia_tensor_bundle_compat` | Optional NVIDIA TensorList conversion components and boundary launch | Yes |
...
These seven packages form a library collection, not an aggregate ROS package.
The standalone CMake project includes only the core and optional CUDA/HIP
backends. NVIDIA detection compositions belong to
[`gpu_ros_nvidia_reference`](../gpu_ros_object_detection/gpu_ros_nvidia_reference/),
not to the conversion package. AMD profiles exclude the conversion package.
...
cmake --build /tmp/gpu-ros-managed-core --parallel
ctest --test-dir /tmp/gpu-ros-managed-core \
  --output-on-failure --no-tests=error
```

From `gpu_ros_managed/`, use `cmake -S .` with the same options. Enable a
backend only when its SDK is installed:

```bash
cmake -S gpu_ros_managed -B /tmp/gpu-ros-managed-hip \
  -DGPU_ROS_MANAGED_BUILD_TESTING=ON \
  -DGPU_ROS_MANAGED_BUILD_CUDA=OFF \
  -DGPU_ROS_MANAGED_BUILD_HIP=ON \
  -DGPU_ROS_MANAGED_HIP_REQUIRE_SDK=ON
cmake --build /tmp/gpu-ros-managed-hip --parallel
ctest --test-dir /tmp/gpu-ros-managed-hip --output-on-failure

cmake -S gpu_ros_managed -B /tmp/gpu-ros-managed-cuda \
  -DGPU_ROS_MANAGED_BUILD_TESTING=ON \
  -DGPU_ROS_MANAGED_BUILD_CUDA=ON \
  -DGPU_ROS_MANAGED_BUILD_HIP=OFF
cmake --build /tmp/gpu-ros-managed-cuda --parallel
ctest --test-dir /tmp/gpu-ros-managed-cuda --output-on-failure
```

The device tests use CTest skip code 77 when no compatible device is available.
A missing SDK can leave its backend out of the build. The HIP require-SDK
option makes a missing HIP SDK a configure error. A skip is not a device-test
PASS.

## Build in a ROS 2 workspace

Pass individual package directories to colcon. The parent directory contains
the standalone CMake project and must not be discovered as another package.
...
packages=(
  ...
  gpu_ros_managed/gpu_ros_managed_ros
...
colcon build --symlink-install --base-paths "${packages[@]}" \
  --packages-select \
    ...
    gpu_ros_tensor_bundle_msgs gpu_ros_managed_tensor_bundle
source install/setup.bash
```

Replace the HIP package path and selection with `gpu_ros_managed_cuda` for a
CUDA workspace. Omit the message and TensorBundle packages when building only
the ROS wrappers. Detection builds use the explicit package lists in
[`gpu_ros_object_detection/docker/`](../gpu_ros_object_detection/docker/).
...
The detection collection uses HIP and ONNX Runtime MIGraphX for its AMD
managed path. [Experiment methods](../skills/gpu-ros-experiments/) describe
runtime setup, fixed-input comparison, and explicitly requested benchmarks.
[Result records](../docs/results/README.md) summarize current observations and
provenance; this collection remains a pre-release source project.

## Contributing

Test ownership, readiness, error, and shutdown behavior when those contracts
change. Run CTest in the matching standalone build directory. Backend checks
need their SDK and skip with code 77 when no compatible device is available.

For ROS changes, source ROS and the built workspace overlay, then use the
individual package paths from the build example:
...
Run this after the corresponding build; a merged install is not required.
