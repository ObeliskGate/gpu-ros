...
BufferState::~BufferState()
{
  ...
  if (producer_event != 0) {
    events.push_back(producer_event);
  }
...
{
  if (!stream.state_) {
    throw std::invalid_argument("DeviceStream is empty");
  }
...
{
BackendKind DeviceStream::backend_kind() const
{
  return detail::StreamAccess::get(*this).device.backend;
}
DeviceId DeviceStream::device_id() const
{
  return detail::StreamAccess::get(*this).device;
}
...
uint8_t * WriteHandle::data() const noexcept
{
  return state_ ? state_->data : nullptr;
}
size_t WriteHandle::size() const noexcept
{
...
{
  if (!owner) {
    throw std::invalid_argument("WriteHandle owner is null");
  }
  if (!responsible_ || !state_) {
    throw std::logic_error("WriteHandle is no longer active");
  }
...
ReadHandle::ReadHandle(std::shared_ptr<detail::BufferState> state, DeviceStream stream)
...
const uint8_t * ReadHandle::data() const noexcept
{
  return state_ ? state_->data : nullptr;
}
size_t ReadHandle::size() const noexcept
{
  return state_ ? state_->size : 0;
}
...
const uint8_t * BlockingReadyLease::data() const noexcept
{
  return state_ ? state_->data : nullptr;
}
size_t BlockingReadyLease::size() const noexcept
{
  return state_ ? state_->size : 0;
}

size_t DeviceBuffer::size() const noexcept
{
  return state_->size;
}
DeviceId DeviceBuffer::device_id() const noexcept
{
...
{
  std::lock_guard<std::mutex> lock(state_->mutex);
  return state_->phase == detail::BufferPhase::kReady ? state_->readiness
                                                      : BufferReadiness::kNotReady;
}
...
SynchronizedWriteHandle DeviceBuffer::get_synchronized_write_handle()
{
  std::lock_guard<std::mutex> lock(state_->mutex);
  if (state_->phase != detail::BufferPhase::kFresh) {
    throw std::logic_error("DeviceBuffer synchronized writer can only be acquired once");
  }
  state_->phase = detail::BufferPhase::kWriting;
...
void DeviceBuffer::copy_from_host_blocking(const void * source, size_t bytes)
{
  ...
  try {
    if (bytes != 0) {
      state_->ops->copy_host_to_device(state_->device.ordinal, state_->data, source, bytes);
    }
    state_->readiness = BufferReadiness::kSynchronouslyReady;
    state_->phase = detail::BufferPhase::kReady;
  } catch (...) {
...
void DeviceBuffer::copy_to_host_blocking(void * destination, size_t bytes) const
{
  ...
  if (bytes != 0) {
    state_->ops->copy_device_to_host(state_->device.ordinal, destination, lease.data(), bytes);
  }
}

DeviceStream detail::DeviceBufferFactory::make_stream(DeviceId device, detail::NativeStream native,
  std::shared_ptr<void> owner, std::shared_ptr<detail::BackendOps> ops)
{
  ...
  std::shared_ptr<void> owner, std::shared_ptr<detail::BackendOps> ops, detail::BufferPhase phase)
{
  ...
  if (size != 0 && data == nullptr) {
    throw std::invalid_argument("Device allocation is null");
  }
  if (size != 0 && !owner) {
