...
#include "gpu_ros_rtdetr/rtdetr_managed_hip_nodes.hpp"
...
void CheckHip(hipError_t error, const char * operation)
{
  ...
  throw std::runtime_error(
    std::string(operation) + ": " + hipGetErrorName(error) + " (" + hipGetErrorString(error) + ")");
}
...
std::shared_ptr<gpu_ros_managed::DeviceBuffer> RequireDeviceTensor(
  const gpu_ros_managed::ManagedTensor & tensor, const std::string & name,
  gpu_ros_managed::TensorDataType dtype, int device_id)
{
  ...
  if (!buffer || buffer->device_id() !=
                   gpu_ros_managed::DeviceId{gpu_ros_managed::BackendKind::kHip, device_id})
  {
  ...
  const std::vector<int64_t> & expected, const std::string & name)
{
  if (tensor.shape() != expected ||
      tensor.byte_size() != gpu_ros_managed::tensor_byte_size(expected,
      ...
      publisher_(this, "managed_tensor_output", rclcpp::QoS(10))
{
  ...
  const size_t output_bytes =
    CheckedProduct(CheckedProduct(kChannels, static_cast<size_t>(output_width_), "output tensor"),
  ...
  subscription_ = create_subscription<sensor_msgs::msg::Image>("image", 10,
  ...
  sensor_msgs::msg::Image::ConstSharedPtr message)
{
  ...
  try {
    ...
    const auto plan = detection_common::MakeImagePreprocessPlan(*message, output_width_,
      output_height_, detection_common::PreprocessNormalization::kUnitRange);
    const size_t source_bytes = CheckedProduct(
      static_cast<size_t>(plan.input_step), static_cast<size_t>(plan.input_height), "source image");
    ...
    const auto remaining_timeout = [&reservation_deadline]() {
      ...
      return now >= reservation_deadline
    ...
    gpu_ros_managed::ManagedTensor tensor(tensor_name_, gpu_ros_managed::TensorDataType::kFloat32,
    ...
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
      "RT-DETR Managed HIP encoder dropped frame (%zu pool drops): %s", pool_exhaustion_drops_,
      error.what());
  }
}
      ...
      publisher_(this, "managed_tensor_output", rclcpp::QoS(10))
{
  ...
  subscriber_ =
    std::make_unique<gpu_ros_managed::ManagedSubscriber<gpu_ros_managed::ManagedTensorBundleView>>(
      this, "managed_tensor_input",
      std::bind(&RtDetrManagedHipPreprocessorNode::InputCallback, this, std::placeholders::_1),
      rclcpp::QoS(10));
}
  ...
  gpu_ros_managed::ManagedTensorBundleView message)
{
  ...
  try {
    ...
    const auto input_buffer = RequireDeviceTensor(
      input, input_image_tensor_name_, gpu_ros_managed::TensorDataType::kFloat32, gpu_device_id_);
    if (input.shape() != std::vector<int64_t>{1, 3, model_input_height_, model_input_width_}) {
    ...
    const int64_t original_width =
      use_max_dim_for_orig_size_ ? std::max(image_width_, image_height_) : image_width_;
    const int64_t original_height =
      use_max_dim_for_orig_size_ ? std::max(image_width_, image_height_) : image_height_;
    detection_common::LaunchHipWriteInt64Pair(
      reinterpret_cast<int64_t *>(size_block->writer.data()), original_width, original_height,
      stream_.get());
    gpu_ros_managed::ManagedTensor image_out(output_image_tensor_name_,
      gpu_ros_managed::TensorDataType::kFloat32, input.shape(), input_buffer);
    gpu_ros_managed::ManagedTensor size_out(output_size_tensor_name_,
      gpu_ros_managed::TensorDataType::kInt64, {1, 2}, size_block->buffer);
    size_block->writer.finalize();
    publisher_.publish(gpu_ros_managed::ManagedTensorBundle(message.get().header(),
      std::vector<gpu_ros_managed::ManagedTensor>{std::move(image_out), std::move(size_out)}));
  } catch (const std::exception & error) {
    if (size_block) {
      size_block->writer.fail();
    }
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
      "RT-DETR Managed HIP preprocessor dropped frame (%zu pool drops): %s", pool_exhaustion_drops_,
      error.what());
  }
}

RtDetrManagedHipDecoderNode::RtDetrManagedHipDecoderNode(const rclcpp::NodeOptions options)
      ...
      read_stream_(gpu_ros_managed::hip::make_stream(declare_parameter<int>("gpu_device_id", 0)))
{
  ...
  subscriber_ =
    std::make_unique<gpu_ros_managed::ManagedSubscriber<gpu_ros_managed::ManagedTensorBundleView>>(
      this, "managed_tensor_input",
      std::bind(&RtDetrManagedHipDecoderNode::InputCallback, this, std::placeholders::_1),
      rclcpp::QoS(10));
}

void RtDetrManagedHipDecoderNode::InputCallback(gpu_ros_managed::ManagedTensorBundleView message)
{
  try {
    CheckHip(hipSetDevice(read_stream_.stream().device_id().ordinal),
    ...
    const auto labels_buffer = RequireDeviceTensor(labels, config_.labels_tensor_name,
      gpu_ros_managed::TensorDataType::kInt64, read_stream_.stream().device_id().ordinal);
    const auto boxes_buffer = RequireDeviceTensor(boxes, config_.boxes_tensor_name,
      gpu_ros_managed::TensorDataType::kFloat32, read_stream_.stream().device_id().ordinal);
    const auto scores_buffer = RequireDeviceTensor(scores, config_.scores_tensor_name,
      gpu_ros_managed::TensorDataType::kFloat32, read_stream_.stream().device_id().ordinal);
    ...
    CheckHip(hipMemcpyAsync(labels_host.data(), labels_read.data(), labels.byte_size(),
               hipMemcpyDeviceToHost, read_stream_.get()),
      "RT-DETR Managed labels D2H");
    CheckHip(hipMemcpyAsync(boxes_host.data(), boxes_read.data(), boxes.byte_size(),
               hipMemcpyDeviceToHost, read_stream_.get()),
      "RT-DETR Managed boxes D2H");
    CheckHip(hipMemcpyAsync(scores_host.data(), scores_read.data(), scores.byte_size(),
               hipMemcpyDeviceToHost, read_stream_.get()),
      "RT-DETR Managed scores D2H");
    CheckHip(hipStreamSynchronize(read_stream_.get()), "RT-DETR Managed decoder read sync");
    labels_read.finish();
    boxes_read.finish();
    scores_read.finish();
    publisher_->publish(DecodeRtDetrValues(message.header(), labels_host.data(), labels_host.size(),
      boxes_host.data(), boxes_host.size(), scores_host.data(), scores_host.size(), config_));
  } catch (const std::exception & error) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(gpu_ros::rtdetr::RtDetrManagedHipImageEncoderNode)
RCLCPP_COMPONENTS_REGISTER_NODE(gpu_ros::rtdetr::RtDetrManagedHipPreprocessorNode)
RCLCPP_COMPONENTS_REGISTER_NODE(gpu_ros::rtdetr::RtDetrManagedHipDecoderNode)
