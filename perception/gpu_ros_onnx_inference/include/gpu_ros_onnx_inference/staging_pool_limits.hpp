...
inline size_t ValidateStagingPoolAddition(
  size_t tensor_bytes,
  size_t pool_capacity,
  size_t current_cache_bytes,
  size_t current_cache_entries,
  ...
  size_t max_cache_entries)
{
  if (tensor_bytes == 0U || tensor_bytes > max_tensor_bytes) {
    throw std::runtime_error(
            "tensor requires " + std::to_string(tensor_bytes) +
            " bytes; adjust managed_max_tensor_bytes if this shape is expected");
  }
  if (current_cache_entries >= max_cache_entries) {
    throw std::runtime_error(
            "dynamic staging pool cache reached managed_pool_cache_max_entries=" +
  ...
  if (next_cache_bytes > max_cache_bytes) {
    throw std::runtime_error(
            ...
            " bytes; adjust managed_pool_cache_max_bytes=" +
            std::to_string(max_cache_bytes));
  }
