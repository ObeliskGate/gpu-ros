...
def _is_copy_record(event: Mapping[str, Any], size: Optional[int]) -> bool:
    ...
    direction = _text(
        event_field(event, 'direction', 'copy_direction', 'transfer_direction', default='')
    )
    source = event_field(
        event,
        'SrcMemKd',
...
def normalize_event(event: Mapping[str, Any], platform: str = 'generic') -> Dict[str, Any]:
    ...
    if _is_copy_record(event, size):
        ...
        kind = _text(
            event_field(event, 'kind', 'category', 'record_type', 'domain', 'type', default='')
        ).lower()
        kernel_name = _text(
            event_field(
        ...
        if (
            'kernel' in kind
            or 'dispatch' in kind
            or event_field(event, 'kernel_name', 'dispatch_name', default=None) is not None
        ):
            event_type = 'kernel'
...
def load_trace_records(paths: Sequence[Path]) -> List[Mapping[str, Any]]:
    ...
    for path in paths:
        ...
        try:
            ...
            raise ValueError(
                f'{path}: report is not pure JSON; first line: {first_line!r}'
            ) from exc
...
def serialize_counter(counter: Counter) -> List[Dict[str, Any]]:
    ...
    return [
        {'key': _signature_json(key), 'count': counter[key]} for key in sorted(counter, key=str)
    ]
...
def summarize_events(
    ...
    kernels = Counter(event['operation'] for event in normalized if event['event_type'] == 'kernel')
    copies = Counter(
        signature(event) for event in normalized if event['event_type'] == 'memory_copy'
    )
...
def build_pair_report(
    ...
    missing_adapter_directions = (
        sorted(adapter_directions - set(managed['memory_totals']))
        if require_adapter_directions
        else []
    )
    ...
    trace_data_complete = (
        profiler_complete
        and has_explainable_events(reference)
        and has_explainable_events(managed)
        and not incomplete_memory_copy_evidence
    )
    ...
    if reference_frames is not None:
        payload_keys = sorted(
            set(reference['payload_copy_counts']) | set(managed['payload_copy_counts']), key=str
        )
        for key in payload_keys:
            ...
            within_limit = rate_delta <= max_payload_copy_rate_delta or _is_adapter_direction(
                direction, adapter_directions
            )
    ...
    memory_directions = sorted(set(reference['memory_totals']) | set(managed['memory_totals']))
    memory_total_deltas = []
    ...
    unresolved_payload_copy_risk = [
        ...
        if (
            difference['name'] in explicit_risk_names
            or looks_like_payload_kernel(difference['name'])
        )
    ]
    ...
    for item in confirmed_boundary:
        memory_copy_failures.append(
            {
                'reason': (
                    'pointer/lifetime and trace evidence confirmed a Managed boundary payload copy'
                ),
                'evidence': item,
            }
        )
    ...
    boundary_payload_copy_evidence.extend(
        ...
        for failure in memory_copy_failures
        if 'tensor-sized' in failure.get('reason', '')
    )

    if memory_copy_failures:
        status = 'FAIL'
    elif (
        not trace_data_complete
        or pointer_lifetime_complete is False
        or unresolved_payload_copy_risk
    ):
        status = 'INCONCLUSIVE'
    else:
        status = 'PASS'
    ...
    managed_extra_memcopies = [
        {'signature': _signature_json(sig), 'count': count} for sig, count in managed_extra
    ]
