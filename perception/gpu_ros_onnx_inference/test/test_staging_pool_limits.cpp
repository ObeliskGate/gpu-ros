...
{
  EXPECT_THROW(
    detail::ValidateStagingPoolAddition(65, 1, 0, 0, 64, 1000, 2),
    std::runtime_error);
}
...
{
  EXPECT_THROW(
    detail::ValidateStagingPoolAddition(1, 1, 0, 2, 64, 1000, 2),
    std::runtime_error);
}
...
{
  EXPECT_THROW(
    detail::ValidateStagingPoolAddition(64, 4, 800, 1, 64, 1000, 2),
    std::runtime_error);
}
...
{
  EXPECT_THROW(
    detail::ValidateStagingPoolAddition(
      std::numeric_limits<size_t>::max(), 2, 0, 0,
...
TEST(StagingPoolLimits, RejectsAdditionOverflow)
{
  EXPECT_THROW(
    detail::ValidateStagingPoolAddition(
      1, 1, std::numeric_limits<size_t>::max(), 0,
