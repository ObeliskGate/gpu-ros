  ...
  sensor_msgs::msg::Image::ConstSharedPtr message)
{
  ...
  try {
    ...
    RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 5000,
      "YOLOv8 Managed HIP encoder dropped frame (%zu pool drops): %s", pool_exhaustion_drops_,
      error.what());
  }
}

YoloV8ManagedHipDecoderNode::YoloV8ManagedHipDecoderNode(const rclcpp::NodeOptions options)
      ...
      read_stream_(gpu_ros_managed::hip::make_stream(declare_parameter<int>("gpu_device_id", 0)))
{
  ...
  subscriber_ =
    std::make_unique<gpu_ros_managed::ManagedSubscriber<gpu_ros_managed::ManagedTensorBundleView>>(
      ...
      std::bind(&YoloV8ManagedHipDecoderNode::InputCallback, this, std::placeholders::_1),
      rclcpp::QoS(10));
}

void YoloV8ManagedHipDecoderNode::InputCallback(gpu_ros_managed::ManagedTensorBundleView message)
{
  try {
    ...
    const auto buffer =
      DeviceTensorBuffer(tensor, config_.tensor_name, read_stream_.stream().device_id().ordinal);
    ...
    CheckHip(hipMemcpyAsync(values.data(), read.data(), tensor.byte_size(), hipMemcpyDeviceToHost,
               read_stream_.get()),
      "YOLOv8 Managed D2H output copy");
    CheckHip(hipStreamSynchronize(read_stream_.get()), "YOLOv8 Managed decoder read sync");
    read.finish();
    publisher_->publish(
      DecodeYoloV8Values(message.header(), values.data(), values.size(), tensor.shape(), config_));
  } catch (const std::exception & error) {
...
RCLCPP_COMPONENTS_REGISTER_NODE(gpu_ros::yolov8::YoloV8ManagedHipImageEncoderNode)
RCLCPP_COMPONENTS_REGISTER_NODE(gpu_ros::yolov8::YoloV8ManagedHipDecoderNode)
