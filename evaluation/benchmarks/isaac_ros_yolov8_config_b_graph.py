...
"""Config B benchmark: TensorRT + standard TensorList + std YOLOv8 decoder."""
...
def launch_setup(container_prefix, container_sigterm_timeout):
    ns = TestIsaacROSYoloV8ConfigB.generate_namespace()
    ...
    tensor_rt_node = ComposableNode(
        ...
        parameters=[{
            'model_file_path': os.path.join(
                TestIsaacROSYoloV8ConfigB.get_assets_root_path(),
...
def generate_test_description():
    model_path = os.path.join(
        TestIsaacROSYoloV8ConfigB.get_assets_root_path(), 'models', common.MODEL_FILE_NAME)
    ...
    return TestIsaacROSYoloV8ConfigB.generate_test_description_with_nsys(launch_setup)


class TestIsaacROSYoloV8ConfigB(ROS2BenchmarkTest):
    """Config B: TensorRT + standard TensorList + std YOLOv8 decoder."""

    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS YOLOv8 (B: TRT + std ROS2)',
