...
def launch_setup(container_prefix, container_sigterm_timeout):
    ns = TestIsaacROSRtDetrManaged.generate_namespace()
    ...
    nitros_to_managed = ComposableNode(
        name='NitrosToManaged', namespace=ns, package='isaac_ros_onnx_inference',
    ...
    onnx = ComposableNode(
        name='OnnxInference', namespace=ns, package='isaac_ros_onnx_inference',
        plugin='nvidia::isaac_ros::onnx_inference::OnnxInferenceNode',
        parameters=[{
            'model_file_path': os.path.join(
                TestIsaacROSRtDetrManaged.get_assets_root_path(), 'models', common.MODEL_FILE_NAME),
    ...
    managed_to_nitros = ComposableNode(
        name='ManagedToNitros', namespace=ns, package='isaac_ros_onnx_inference',
...
def generate_test_description():
    return TestIsaacROSRtDetrManaged.generate_test_description_with_nsys(launch_setup)


class TestIsaacROSRtDetrManaged(ROS2BenchmarkTest):
    """ORT CUDA transport=managed versus Config C NITROS transport."""

    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS RT-DETR (M: ORT CUDA + Managed)',
