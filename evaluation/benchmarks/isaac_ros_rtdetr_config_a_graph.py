...
def launch_setup(container_prefix, container_sigterm_timeout):
    ns = TestIsaacROSRtDetrConfigA.generate_namespace()
...
def generate_test_description():
    model_path = os.path.join(
        TestIsaacROSRtDetrConfigA.get_assets_root_path(), 'models', common.MODEL_FILE_NAME)
    ...
    return TestIsaacROSRtDetrConfigA.generate_test_description_with_nsys(launch_setup)


class TestIsaacROSRtDetrConfigA(ROS2BenchmarkTest):
    """Config A: TensorRT + NITROS (baseline)."""

    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS RT-DETR (A: TRT + NITROS)',
