...
def launch_setup(container_prefix, container_sigterm_timeout):
    ns = TestIsaacROSYoloV8ConfigC.generate_namespace()
    ...
    onnx_node = ComposableNode(
        ...
        parameters=[{
            'model_file_path': os.path.join(
                TestIsaacROSYoloV8ConfigC.get_assets_root_path(),
...
def generate_test_description():
    return TestIsaacROSYoloV8ConfigC.generate_test_description_with_nsys(launch_setup)


class TestIsaacROSYoloV8ConfigC(ROS2BenchmarkTest):
    """Config C: ONNX Runtime CUDA + NITROS + upstream YOLOv8 decoder."""

    config = ROS2BenchmarkConfig(
        benchmark_name='Isaac ROS YOLOv8 (C: ORT + NITROS)',
