...
RESULTS_FILE = os.environ.get("R2B_RESULT_FILE") or "gpu_ros_yolov8_phase2a_amd.json"
...
def launch_setup(container_prefix, container_sigterm_timeout):
    namespace = TestGpuRosYoloV8Phase2aAmd.generate_namespace()
    model_path = model_path_for_test(TestGpuRosYoloV8Phase2aAmd)
...
def generate_test_description():
    return TestGpuRosYoloV8Phase2aAmd.generate_test_description_with_nsys(launch_setup)


class TestGpuRosYoloV8Phase2aAmd(ROS2BenchmarkTest):
    """Phase 2a AMD target: ONNX Runtime MIGraphX + standard ROS2 transport."""

    config = ROS2BenchmarkConfig(
        benchmark_name="GPU ROS YOLOv8 Phase 2A AMD (ORT MIGraphX + std ROS2)",
