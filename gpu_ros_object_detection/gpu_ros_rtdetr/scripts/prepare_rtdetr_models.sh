...
# NVIDIA/reference-only helper for the historical SyntheticaDETR TensorRT
# matrix. The AMD default uses the external RT-DETRv2 asset manager and never
# invokes this script.
...
if [[ ! -f "${FP32_ONNX}" ]]; then
  ...
  echo "Import the user-acquired external asset first (see skills/gpu-ros-experiments/references/README.md)." >&2
  exit 1
fi
...
if [[ -f "${FP16_ONNX}" ]]; then
  ...
  uv run --no-project python "${SCRIPT_DIR}/convert_rtdetr_fp16.py" \
    --input "${FP32_ONNX}" --output "${FP16_ONNX}"
fi
