...
CONTAINER_ENTRYPOINT="${GPU_ROS_REPO_ROOT}/gpu_ros_object_detection/docker/phase2a-amd-entrypoint.sh"
...
require_explicit_apptainer_environment() {
  ...
  for name in \
    ...
    [[ -n "${!name:-}" ]] || die \
      "${name} is required for Apptainer work; set the public AMD runtime variables documented in skills/gpu-ros-experiments/references/amd-runtime-contract.md"
  done
...
usage() {
  cat >&2 <<'EOF'
Usage: gpu_ros_object_detection/docker/phase2-amd.sh {bootstrap|build|up|colcon|verify|preflight|shell|stop|down}
...
repo_check() {
  ...
  [[ -f "${ROOT_DIR}/gpu_ros_managed/gpu_ros_managed_core/package.xml" ]] ||
    die "managed package manifest is missing: ${ROOT_DIR}/gpu_ros_managed/gpu_ros_managed_core/package.xml"
  [[ -f "${ROOT_DIR}/gpu_ros_managed/gpu_ros_tensor_bundle_msgs/package.xml" ]] ||
    die "interface package manifest is missing: ${ROOT_DIR}/gpu_ros_managed/gpu_ros_tensor_bundle_msgs/package.xml"
...
run_phase2() {
  if [[ "${RUNTIME}" == docker ]]; then
    docker_exec "${OVG_WORKSPACE_ROOT}/gpu_ros_object_detection/tools/phase2" "$@"
  else
