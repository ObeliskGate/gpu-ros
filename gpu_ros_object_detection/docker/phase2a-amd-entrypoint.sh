...
if [[ ! -f "${COLCON_DEFAULTS_FILE}" ]]; then
  if [[ "${BASH_SOURCE[0]}" == "${REPO_ROOT}/gpu_ros_object_detection/docker/phase2a-amd-entrypoint.sh" ]]; then
...
export PATH="${REPO_ROOT}/gpu_ros_object_detection/tools:${PATH:-/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin}"
