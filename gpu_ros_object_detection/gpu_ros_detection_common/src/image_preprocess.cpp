...
ImageEncoding ParseEncoding(const std::string & encoding)
{
  ...
  if (encoding == enc::RGB8) {
    return ImageEncoding::kRgb8;
  }
  if (encoding == enc::BGR8) {
    return ImageEncoding::kBgr8;
  }
  if (encoding == enc::RGBA8) {
    return ImageEncoding::kRgba8;
  }
  if (encoding == enc::BGRA8) {
    return ImageEncoding::kBgra8;
  }
  if (encoding == enc::MONO8) {
    return ImageEncoding::kMono8;
  }
...
{
  return encoding == ImageEncoding::kMono8
           ? 1U
...
void ValidateImageStorage(const sensor_msgs::msg::Image & image, size_t bytes_per_pixel)
{
  ...
  if (bytes_per_pixel != 0U &&
      static_cast<size_t>(image.width) > std::numeric_limits<size_t>::max() / bytes_per_pixel)
  {
  ...
  int64_t output_width, int64_t output_height, PreprocessNormalization normalization)
{
  ...
  const double scale =
    std::min(static_cast<double>(output_width) / static_cast<double>(image.width),
  ...
  const int resized_width =
    static_cast<int>(std::clamp(rounded_width, 1.0, static_cast<double>(output_width)));
  const int resized_height =
    static_cast<int>(std::clamp(rounded_height, 1.0, static_cast<double>(output_height)));
  return ImagePreprocessPlan{encoding, image.width, image.height, image.step, channels,
    static_cast<int>(output_width), static_cast<int>(output_height), resized_width, resized_height,
    normalization};
}
  ...
  const sensor_msgs::msg::Image & image, const ImagePreprocessPlan & plan)
{
  ...
  const float scale =
    plan.normalization == PreprocessNormalization::kUnitRange ? 1.0F / 255.0F : 1.0F;
