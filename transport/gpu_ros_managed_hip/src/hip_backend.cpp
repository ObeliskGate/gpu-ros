...
public:
  ...
  {
    check(hipEventRecord(reinterpret_cast<hipEvent_t>(e), reinterpret_cast<hipStream_t>(s)),
      "hipEventRecord");
  }
  void wait_event(detail::NativeStream s, detail::Event e) override
  {
    check(hipStreamWaitEvent(reinterpret_cast<hipStream_t>(s), reinterpret_cast<hipEvent_t>(e), 0),
      "hipStreamWaitEvent");
  }
  ...
  {
    if (e) {
      static_cast<void>(hipEventDestroy(reinterpret_cast<hipEvent_t>(e)));
    }
  }
  ...
  {
    select_device(id);
    check(hipMemcpy(dst, src, bytes, hipMemcpyHostToDevice), "hipMemcpy H2D");
  }
  void copy_device_to_host(int id, void * dst, const void * src, size_t bytes) override
  {
    select_device(id);
    check(hipMemcpy(dst, src, bytes, hipMemcpyDeviceToHost), "hipMemcpy D2H");
  }
};
...
HipStream make_stream(int id)
{
  ...
  auto owner = std::shared_ptr<void>(reinterpret_cast<void *>(native), [id](void * p) {
    static_cast<void>(hipSetDevice(id));
    static_cast<void>(hipStreamDestroy(reinterpret_cast<hipStream_t>(p)));
  });
  return HipStream(detail::DeviceBufferFactory::make_stream({BackendKind::kHip, id},
...
{
  if (!native) {
    throw std::invalid_argument("Borrowed HIP stream is null");
  }
  return HipStream(detail::DeviceBufferFactory::make_stream({BackendKind::kHip, id},
    reinterpret_cast<detail::NativeStream>(native), std::move(owner), ops()));
}
