...
{
  if (shape.empty()) {
    throw std::invalid_argument("Tensor rank must be at least one");
  }
  size_t count = 1;
...
ManagedTensor::ManagedTensor(std::string name, TensorDataType dtype, std::vector<int64_t> shape,
      ...
      storage_(std::move(storage)), byte_size_(tensor_byte_size(shape_, dtype_))
{
  ...
  const size_t storage_size = std::visit(
    [](const auto & value) {
      if constexpr (std::is_same_v<std::decay_t<decltype(value)>, HostBuffer>) {
        return value.size();
      } else {
        if (!value) {
          throw std::invalid_argument("DeviceBuffer storage is null");
        }
        return value->size();
      }
    },
    storage_);
  if (storage_size < byte_size_) {
    throw std::invalid_argument("Tensor storage is too small");
  }
}

ManagedTensor ManagedTensor::from_host_external(std::string name, TensorDataType dtype,
  std::vector<int64_t> shape, std::shared_ptr<const void> owner, const void * pointer, size_t size,
  std::vector<uint64_t> strides)
{
  return ManagedTensor(std::move(name), dtype, std::move(shape),
    HostBuffer(std::move(owner), pointer, size), std::move(strides));
}
ManagedTensor ManagedTensor::from_host_copy(std::string name, TensorDataType dtype,
  std::vector<int64_t> shape, const void * pointer, size_t size)
{
  return ManagedTensor(std::move(name), dtype, std::move(shape), HostBuffer::copy(pointer, size));
}
    ...
    : header_(std::move(header)), tensors_(std::move(tensors))
{
  ...
  for (const auto & tensor : tensors_) {
    ...
    if (!host && std::get<std::shared_ptr<DeviceBuffer>>(tensor.storage())->device_id() != device) {
